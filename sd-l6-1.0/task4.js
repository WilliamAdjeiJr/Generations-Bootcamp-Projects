...
// Function to delete a user from the JSON server
async function delUser(id) {
  const url = getServerURL() + `/users/${id}`; // Construct the URL to delete a user
  try {
    const response = await fetch(url, {
      method: 'DELETE'
    });

    const deletedUser = await response.json(); // Parse the JSON response
    console.log('User deleted:');
    console.log(deletedUser); // Print the deleted user details to the console
  } catch (error) {
    console.error('Error deleting user:', error); // Handle any errors
  }
}

// Export the getServerURL, listUsers, addUser, and delUser functions
module.exports = {
  getServerURL,
  listUsers,
  addUser,
